...
export function Experience() {
    ...
    const { ref: rC, visible: vC } = useReveal({ threshold: 0.04 });
    ...
    return (
        <section id="experience" className="section-y">
            <div className="wrap">
                ...
                <div ref={rC} className={cn("reveal space-y-8", vC && "in")}>
                    {experiences.map((exp) => {
                        const isCurrent = exp.duration.toLowerCase().includes("present");
                        const expStats = exp.id === "webtrendz-developer" ? [
                            { value: "4+", sub: "Flutter Apps Shipped" },
                            { value: "3", sub: "Payment Integrations" },
                            { value: "4-App", sub: "Migration Lead" },
                            { value: "Serverless", sub: "AWS Lambda / Node.js" },
                        ] : [
                            { value: "15+", sub: "ERP Modules Validated" },
                            { value: "RBAC", sub: "Access Control Layer" },
                            { value: "ApexCharts", sub: "Dashboards Built" },
                            { value: "Redux", sub: "Toolkit Scaled" },
                        ];

                        return (
                            <div
                                key={exp.id}
                                className="rounded-2xl overflow-hidden border border-border bg-bg-card hover:shadow-md transition-shadow duration-300"
                                style={{ boxShadow: "var(--shadow-3)" }}
                            >
                                <div className="grid sm:grid-cols-[1fr_auto] items-start gap-4 px-5 sm:px-8 py-6 border-b border-border bg-bg-2">
                                    <div className="min-w-0">
                                        <p className="t-label text-accent mb-2">{exp.type} · {exp.location}</p>
                                        <h3 className="t-h2 text-fg break-words">{exp.company}</h3>
